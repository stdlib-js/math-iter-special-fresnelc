...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterFresnelc, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterFresnelc( uniform( 0.0, 10.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterFresnelc( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the Fresnel integral C(x) for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
	...
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = fresnelc( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the Fresnel integral C(x) for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': fresnelc( values[ 0 ] ),
			'done': false
		},
		{
			'value': fresnelc( values[ 1 ] ),
			'done': false
		},
		{
			'value': fresnelc( values[ 2 ] ),
			'done': false
		},
		{
			'value': fresnelc( values[ 3 ] ),
			'done': true
		}
	];
